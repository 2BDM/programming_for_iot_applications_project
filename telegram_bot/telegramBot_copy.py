...
class RESTBot:
    ...
    def __init__(self, conf_dict):
        ...
        #####
        self._serv_cat_addr = "http://" + self.catalogIP
        # Registration at service catalog
        self._registered_at_catalog = False
        self.id = conf_dict["telegram"]["id"]
    ...
    def registerAtServiceCatalog(self, max_tries=10):
        ...
        # Actual
        tries = 0
        while not self._registered_at_catalog and tries < max_tries and  # and self.id is not None:            
            tries += 1
            try:
                reg = requests.post(self._serv_cat_addr + '/service', data=json.dumps(self.myDict))
